...
def run(manifest_path: Path) -> None:
    ...
    for number, entry in enumerate(job["files"]):
        ...
        try:
            facts = (
                extractor.extract(entry["path"], content, timeout=job["parse_seconds"])
                if entry["language"] == "python"
                else text_facts(entry["path"], content)
            )
        except CodeError as error:
            if error.code != "parse_timeout":
                raise
            facts = text_facts(entry["path"], content)
            facts["nodes"][0].update(language="python", parse_status="failed")
